"""Training script for fine-tuning the Sentence-Roberta model with cls pooling."""
...
from transformers import get_scheduler, RobertaTokenizer
from sroberta import SentenceRoberta, GteLoss, InfoNCELoss
from torch.optim.lr_scheduler import ReduceLROnPlateau, ChainedScheduler


MODEL_NAME = "Dingyun-Huang/oe-roberta-base"
DATASET_NAME = "Dingyun-Huang/optoelectronics-title-abstract-triplets"
TRAINING_CONFIG = "configs/sroberta_cls.yaml"
...
def setup_scheduler(optimizer, config, training_dataloader):
    ...
    """
    steps = int(len(training_dataloader) * config['epochs']) * config['Accelerate']['num_processes']
...
def main():
    """
    Main function for training the Sentence-Roberta model with cls pooling.
    """
    ...
    #     config['pooler'] = 'dense & tanh & cls pooling'

    for param in model.parameters():
        param.requires_grad = True
        config['requires_grad'] = 'all'
        config['pooler'] = 'linear dense & tanh & cls pooling'
        ...
        for batch in tqdm(training_dataloader, disable=not accelerator.is_local_main_process):
            ...
            if step % 1000 == 0:
                with accelerator.autocast():
                    accelerator.save_state(f"models/{loss_func.__class__.__name__}/2024-06-02-cls/checkpoint.{step}")
